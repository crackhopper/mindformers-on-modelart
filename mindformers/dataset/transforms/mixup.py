...
def rand_bbox_minmax(img_shape, minmax, count=None):
    ...
    """
    if len(minmax) != 2:
        raise ValueError(f"len(minmax) is not 2. ")
    img_h, img_w = img_shape[-2:]
...
class Mixup(PyTensorOperation):
    ...
    def __init__(self, mixup_alpha=1., cutmix_alpha=0., cutmix_minmax=None, prob=1.0, switch_prob=0.5,
        ...
        if self.cutmix_minmax is not None:
            if len(self.cutmix_minmax) != 2:
                raise ValueError(f"len(cutmix_minmax) is not 2.")
    ...
    def _params_per_elem(self, batch_size):
        ...
        if self.mixup_enabled:
            if self.mixup_alpha > 0. and self.cutmix_alpha > 0.:
                ...
                raise ValueError("One of mixup_alpha > 0., cutmix_alpha > 0., cutmix_minmax not None should be true.")
